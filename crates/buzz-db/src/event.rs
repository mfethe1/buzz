...
///
/// # Head-advancement guarantee (contract v3)
///
...
///
/// # Idempotent replay exception (contract v3)
///
...
pub async fn insert_channel_head_checked(
    ...
    // so a byte-identical transport retry must never surface as a false
    // conflict (contract v3).
    if head
        .as_ref()
    ...
    let status = match (precondition, head.as_ref()) {
        ...
        (ChannelHeadPrecondition::ExpectedHead(expected), Some((id, head_created_at))) => {
            if id.as_slice() != expected {
                ...
                // the visible canvas unchanged (contract v3 head-advancement).
...
mod tests {
    ...
    /// Contract v3 idempotent-replay exception: replaying the byte-identical
    /// head succeeds as a no-op even when the supplied precondition would
    /// otherwise conflict (a stale `ExpectNoHead` here). Precondition is not
    /// evaluated for a duplicate, so safe transport retry never becomes a false
    /// conflict.
    #[tokio::test]
    #[ignore = "requires Postgres"]
    ...
    /// Contract v3 head-advancement guarantee: a candidate whose `created_at`
    /// equals the head's but whose id is HIGHER cannot become the head under
    ...
    async fn channel_head_checked_rejects_same_second_higher_id() {
        ...
        // Two events at the same second; order them by id.
    ...
    /// Contract v3 head-advancement guarantee: a candidate at the same second
    /// with a LOWER id does sort strictly ahead of the head, so it advances.
    #[tokio::test]
    #[ignore = "requires Postgres"]
    ...
    /// Contract v3 head-advancement guarantee: a behind-clock writer whose
    /// `created_at` predates the head cannot supersede it, even with a matching
    /// precondition.
    #[tokio::test]
    #[ignore = "requires Postgres"]
    async fn channel_head_checked_rejects_behind_clock_writer() {
        ...
        // Writer's clock is 15 minutes behind — created_at earlier than head.
    ...
    /// Contract v3 race soundness: two concurrent authors both assert the same
    /// live head and both sign strictly-advancing writes. The per-(community,
    /// channel) advisory lock must serialize check+insert so exactly one wins
    /// (`Inserted`) and the other observes the moved head (`RevisionMismatch`).
    /// Exactly one write may become the visible head.
    #[tokio::test]
    #[ignore = "requires Postgres"]
