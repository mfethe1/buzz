...
fn approval_json(approval: &buzz_db::workflow::ApprovalRecord) -> Value {
    serde_json::json!({
        ...
        "message": approval.request_message,
        "definition_hash": approval.definition_hash,
        "request_event_id": approval.request_event_id.as_ref().map(hex::encode),
        "decision_event_id": approval.decision_event_id.as_ref().map(hex::encode),
