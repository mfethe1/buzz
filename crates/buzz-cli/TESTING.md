...
link reads, auth inputs, and exit-code mapping that Berd shells out to. It does
not replace the live-relay command runbook below; it only keeps the CLI
contract from drifting silently in CI.
