...
import 'task.dart';
import 'task_assignee_label.dart';
...
class _TaskDetailView extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final description = detail.task.body?.trim() ?? '';
    final descriptionRunes = description.runes.toList();
    final descriptionLimit = useState(taskDetailBodyChars);
            ...
            Flexible(
              child: SingleChildScrollView(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    if (detail.task.assignee != null)
                      Padding(
                        padding: const EdgeInsets.only(top: Grid.xxs),
                        child: TaskAssigneeLabel(pubkey: detail.task.assignee!),
                      ),
                    if (description.isNotEmpty) ...[
                      ...
                      Text(
                        String.fromCharCodes(
                          descriptionRunes.take(descriptionLimit.value),
                        ),
                        key: const ValueKey('task-detail-description'),
                      ),
                      if (descriptionRunes.length > descriptionLimit.value)
                        TextButton(
                          onPressed: () => descriptionLimit.value *= 2,
